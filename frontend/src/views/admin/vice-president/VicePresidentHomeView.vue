...
const stats = ref({
  ...
  unread_directives: 0,
  archive_count: 0,
  card: { name_ar: '', name_fr: '', photo_url: null },
})

const cardName = computed(() => stats.value.card?.name_ar || stats.value.card?.name_fr || '')
  ...
  <div class="space-y-6">
    ...
    >
      <img
        ...
        :src="cardPhoto"
        alt=""
        class="h-20 w-20 rounded-2xl object-cover object-top"
      />
      <div
        v-else
        class="flex h-20 w-20 items-center justify-center rounded-2xl bg-[var(--rdp-forest)] text-2xl font-bold text-white"
      >
        {{ (cardName || '?').slice(0, 1) }}
      </div>
      <div>
